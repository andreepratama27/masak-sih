...
import { ArrowLeftIcon, MagnifyingGlassIcon, BookmarkIcon } from '@heroicons/react/24/outline'
...
const Navbar = () => {
  ...
  const renderSearchWidget = () => {
    if (location.pathname === '/search') {
      return (
        <>
          <input className='w-full rounded py-2 px-4' placeholder="Cari Menu" onFocus={onSearchFocus} onKeyDown={onSearchEnter} autoFocus />
          <div role="button" className='absolute right-4 text-2xl font-bold text-gray-400' onClick={navigateBack}>
            &times;
          </div>
        </>
      )
    ...
    return (
      <>
        ...
        <div className='absolute right-0 flex items-center gap-4'>
          <MagnifyingGlassIcon className='text-white w-5 h-5' role='button' onClick={() => navigate('/search')} />
  ...
  return (
    <nav className='sticky top-0 z-20 w-full p-4 bg-blue-500 shadow border-0 shadow-md'>
