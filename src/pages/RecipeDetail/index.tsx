import { Tag, Panel, Expander } from "@/components";
...
import { getRecipeDetail } from "@/lib/api/api.endpoints";

export const RecipeDetailLoader = async ({ params }: LoaderFunctionArgs) => {
  try {
    const result = await getRecipeDetail(params.key as string);
...
const RecipeDetail = () => {
  ...
  const renderDescription = (desc: string) => {
    return <Expander content={desc} />;
  };

  return (
    <div className="max-w-md py-8 px-4 mx-auto">
      <section className="content">
        <div className="content-header">
          <img
            src={results?.thumb}
            className="object-cover w-full border-2 border-black h-90"
          />
      ...
      <section className="py-4 content-menu ">
        ...
        <Panel title="Bahan - Bahan">
          <ul className="mt-4">
            {results?.ingredient?.map((item: string, key: number) => (
              <li className="mb-2" key={key}>
                <div className="w-full rounded">- {item}</div>
              </li>
            ))}
          </ul>
        </Panel>
          ...
          <ul className="mt-4">
            {
              results?.step
                ?.map((item, key) => (
                  <li className='mb-2' key={key}>
                    <div className="flex items-center w-full gap-6 p-2 bg-gray-200 rounded break-words" style={{ wordBreak: 'break-all', hyphens: 'auto' }}>
